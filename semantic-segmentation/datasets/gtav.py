...
#     """
#     "/data/changty/DG-segment-dataset/gtav/image/train/00001.png"
#     for c in cities:
...
def make_dataset(mode, maxSkip=0, cv_split=0, combine_all=False):
    """
    Assemble list of images + mask files
    ...
    assert mode in ['train', 'val', 'test', 'trainval', 'all']
    list_path = './split_data/'
    item_list_filepath = os.path.join(list_path, 'gtav_split_' + mode + ".txt")
    if not os.path.exists(item_list_filepath):
        raise Warning("split must be train/val/trainval/test/all")

    item_names = [id.strip() for id in open(item_list_filepath)]

    if combine_all:
        item_list_filepath = os.path.join(list_path, 'gtav_split_' + 'val' + ".txt")
        if not os.path.exists(item_list_filepath):
            raise Warning("split must be train/val/trainval/test/all")

        item_names_val = [id.strip() for id in open(item_list_filepath)]

        item_list_filepath = os.path.join(list_path, 'gtav_split_' + 'test' + ".txt")
        if not os.path.exists(item_list_filepath):
            raise Warning("split must be train/val/trainval/test/all")

        item_names_test = [id.strip() for id in open(item_list_filepath)]

        item_names = item_names + item_names_val + item_names_test
    ...
    for mode in modes:
        # logging.info('{} fine cities: '.format(mode) + str(cv_splits[cv_split][mode]))
        add_items(items, aug_items, item_names, img_path, mask_path,
                    mask_postfix, mode, maxSkip)
...
class GTAV(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img, mask = Image.open(img_path).convert('RGB'), imageio.imread(mask_path)
...
class GTAVUniform(data.Dataset):
    ...
    def __init__(self, mode, maxSkip=0, joint_transform_list=None, sliding_crop=None,
                 ...
                 extract_feature=False, envs_index_list=None):
        self.mode = mode
        self.maxSkip = maxSkip
        ...
        self.image_in = image_in
        self.extract_feature = extract_feature
        # The inferred environment index list for each training sample
        self.envs_index_list = envs_index_list
    ...
    def __getitem__(self, index):
        ...
        # # the RGB image
        # img_org = (img * torch.tensor([0.229, 0.224, 0.225]).unsqueeze(-1).unsqueeze(-1) + torch.tensor([0.485, 0.456, 0.406]).unsqueeze(-1).unsqueeze(-1)) * 255

        if self.envs_index_list != None:
            return img, mask, img_name, mask_aux, self.envs_index_list[index]
        else:
            return img, mask, img_name, mask_aux, 0
