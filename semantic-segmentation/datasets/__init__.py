...
def setup_loaders(args):
    ...
    # adjust the batchsize for Environment Inference stage
    try:
        if args.train_batch_size_ei != 0:
            args.train_batch_size = args.train_batch_size_ei
    except:
    ...
    if 'gtav' in args.dataset:
        ...
        if args.class_uniform_pct:
            ...
            train_set = dataset.GTAVUniform(
                ...
                coarse_boost_classes=coarse_boost_classes,
                image_in=args.image_in,
                envs_index_list=args.envs_index_list)
        else:
            train_set = gtav.GTAV(
                gtav_mode, 0,
                joint_transform=train_joint_transform,
    ...
    train_loader = DataLoader(train_set, batch_size=args.train_batch_size,
                              num_workers=args.num_workers, shuffle=(train_sampler is None), drop_last=args.train_droplast, sampler = train_sampler)
